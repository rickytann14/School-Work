...
/// in Fall 2016 was to implement the missing code in Linkedlist.cpp.
/// Program 5 is to convert all instances of LinkedList to std::list
/// and all zero terminated strings to std::string. Your solution must
/// use only the files defines.h and main.cpp.
///
/// Class members can obtain the source code from the CSE machines
/// by cd-ing to ~ian/csce2100/program5/blank. Copy all of the
...
typedef struct roomContents {
	int room;
	int item;
} roomContents;

eItem roomContents[NUM_ROOMS]; ///< List of items in each room.
...
void initialize(){
  //strncpy(roomContents[OUTSIDE_ROOM],"FROG_ITEM", sizeof roomContents[OUTSIDE_ROOM]);
...
int main(){
  ...
  string myStuff; //items that the player is carrying
